...
export default function Invoice({ searchParams }: {
  searchParams: {
    custID: string, ordID: string
  }
}) {
  const [invoiceData, setinvoiceData] = useState<orderInptype>();
  const [loading, setloading] = useState<boolean>(true);
  useEffect(() => {

    const userOrderData = async () => {
      setloading(true)
      await fetch(`/api/invoice?custID=${searchParams.custID}&ordID=${searchParams.ordID}`, {
        method: 'GET',
        cache: 'no-store',
      }).then(res => res.json())
        .then(res => {
  ...
  return (
    <>
      <div className="bg-[#013088]">
        <div className="w-full px-4 sm:px-6 lg:px-8 mx-auto ">
          <div className="w-full mx-auto">
            <div className="flex flex-col p-4 sm:p-10 hadow-md rounded-xl bg-white text-black">
              <div className="flex justify-between">
                <div>
                  <img src="/img/sppl-dark-logo.png" alt="logo" className="w-[200px]" />
                </div>

                <div className="text-right">
                  <h2 className="text-2xl md:text-3xl font-semibold text-gray-900">Invoice #</h2>
                  <span className="mt-1 block text-gray-800">{invoiceData?.sppl_orderid}</span>
              ...
              <div className="mt-8 grid sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <h3 className="text-lg font-semibold text-gray-800 ">{invoiceData?.name}</h3>
                  <address className="mt-2 not-italic text-gray-500">
                    {invoiceData?.address}<br />
                    {invoiceData?.city} {invoiceData?.state}<br />
                    {invoiceData?.pincode}<br />
                  </address>
                </div>
                ...
                <div className="sm:text-right space-y-2">
                  <div className="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
                    <dl className="grid sm:grid-cols-5 gap-x-3">
                      ...
                      <dd className="col-span-2 text-gray-500">{invoiceData?.paymentdate}</dd>
                    </dl>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6">
                <div className="border border-gray-200 p-4 rounded-lg space-y-4 dark:border-gray-700">
                  <div className="hidden sm:grid sm:grid-cols-5">
                    ...
                    <div className="text-center text-xs font-medium text-gray-500 uppercase">Rate</div>
                  ...
                  {
                    invoiceData?.orderprod.map((item: any, index: number) => (
                    <div key={index} className="grid grid-cols-5 gap-2">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-gray-800 ">₹{item.productsaleprice}</p>
                      </div>
                      <div>
                        <h5 className="sm:hidden text-xs font-medium text-gray-500 uppercase">Amount</h5>
                        <p className="sm:text-right text-gray-800 ">₹{item.quantity * item.productsaleprice}</p>
                      </div>
                    </div>
                    ))
                  }
                ...
                <div className="w-full max-w-2xl sm:text-right space-y-2">
                  <div className="grid grid-cols-2 sm:grid-cols-1 gap-3 sm:gap-2">
                    <dl className="grid sm:grid-cols-5 gap-x-3">
                      ...
                      <dd className="col-span-2 text-gray-500">{invoiceData?.totalbill}</dd>
                    </dl>
